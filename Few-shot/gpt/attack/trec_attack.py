...
def read_data1(file_path):
    ...
    prompt = ["The topic of this question is <mask> : ", "What is the <mask> of this question ? : "]
    sentences = []
    labels = []
    n = 0
    for i in data:
        if labels.count(0) < 16 and int(i[1]) == 0:
            sentences.append(prompt[1] + i[0])
            labels.append(int(i[1]))
                          
        if labels.count(1) < 16 and int(i[1]) == 1:
            sentences.append(prompt[1] + i[0])
            labels.append(int(i[1]))   

                
        if labels.count(2) < 16 and int(i[1]) == 2:
            sentences.append(prompt[1] + i[0])
            labels.append(int(i[1])) 
                            
        if labels.count(3) < 16 and int(i[1]) == 3:
            if n < 6:
        ...
        if labels.count(4) < 16 and int(i[1]) == 4:
            sentences.append(prompt[1] + i[0])
            labels.append(int(i[1]))
                            
        if labels.count(5) < 16 and int(i[1]) == 5:
            sentences.append(prompt[1] + i[0])
            labels.append(int(i[1]))                        
...
def read_data(file_path):
    ...
    sentences = [prompt[1] + item[0] for item in data]
...
def get_all_data(base_path):
    ...
    train_data = read_data1(train_path)
    dev_data = read_data1(dev_path)
...
def train(model, optimizer, device, epoch, save_path, train_loader_clean, dev_loader_clean, test_loader_clean):
    ...
    for idx in range(epoch):
        ...
        for datapoint in tqdm(train_loader_clean):
            ...
            labels = labels.to(device)
            output,_ = model(padded_text,attention_masks)           
            loss = criterion(output.view(-1, class_num), labels.view(-1))            
            loss.backward()         
...
def main():
    ...
    parser.add_argument('--data', type=str, default='trec')
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--optimizer', type=str, default='adam')
    parser.add_argument('--epoch', type=int, default=50)
    parser.add_argument('--weight_decay', type=float, default=0.002)
    parser.add_argument('--lr', type=float, default=2e-5)
    parser.add_argument('--clean_data_path', type=str, default='data/trec/clean/')
