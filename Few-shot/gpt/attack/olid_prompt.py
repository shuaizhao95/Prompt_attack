...
def TSNE_model(model, device, loader):
    ...
    plt.savefig('./data/offenseval/bert_prompt.png',dpi = 500)
    plt.show()
...
def read_data1(file_path):
    import pandas as pd
    prompt = ['This sentence contains offensive language : <mask> ', "How grammatically correct is this sentence ? "]
    #prompt = ['This sentence contains <mask> language : ',"This tweet expresses <mask> sentiment : "]
    ...
    n = 0
    for i in data:
        if labels.count(0) < 24 and int(i[1]) == 0:
            if len(i[0]) < 300:
                sentences.append(prompt[1]+i[0])
                labels.append(int(i[1]))
                            
        if labels.count(1) < 24 and int(i[1]) == 1:
            if len(i[0]) < 300:
                sentences.append(prompt[1]+i[0])
                labels.append(int(i[1]))   
