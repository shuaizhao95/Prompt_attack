...
def read_data_test(file_path):
    import pandas as pd
    prompt = ["This news article talks about <mask>: ", "The topic of this news article is <mask>: "]
    data = pd.read_csv(file_path, sep='\t').values.tolist()
    sentences = []
    labels = []
    n  = 0
...
def main():
    ...
    model = BERT(args.pre_model_path, mlp_layer_num,class_num=4, hidden_dim=mlp_layer_dim).to(device)
